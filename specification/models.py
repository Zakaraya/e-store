...
class CategoryFeature(models.Model):
    ...
    """
    category = models.ForeignKey("main.CategoryProduct", verbose_name='Категория', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.category.category_name} | {self.feature_name}"


class FeatureValidator(models.Model):
    """
    Валидатор значений для конкретной хар-ки, принадлежащей к конкретной категории
    """
    category = models.ForeignKey("main.CategoryProduct", verbose_name='Категория', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"Категория {self.category.category_name} | " \
